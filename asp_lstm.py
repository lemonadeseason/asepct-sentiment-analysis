...
class LSTMClassifier(nn.Module):
    def __init__(self, args, emb_matrix=None):
        super().__init__()
        in_dim = args.rnn_hidden*2 if args.bidirect else args.rnn_hidden
        self.args = args
...
class LSTMAbsaModel(nn.Module):   #aspect-based sentiment analysis
    def __init__(self, args, emb_matrix=None):
        ...
        #asp_lstm模型是三种embedding，lstm+position是两种
    ...
    def forward(self, inputs):
        ...
        asp_wn = mask.sum(dim=1).unsqueeze(-1)                        # aspect words num(求每一个句子的aspect长度，并用list再括起来)
        mask = mask.unsqueeze(-1).repeat(1,1,self.args.rnn_hidden*2 if self.args.bidirect else self.args.rnn_hidden)    # mask for h
        outputs = (h*mask).sum(dim=1) / asp_wn                        # mask h   （点乘）
...
class LSTM(nn.Module):   #还包含rnn
    def __init__(self, args, embeddings, mem_dim, num_layers):
        ...
        self.in_dim = args.emb_dim+args.pos_dim+args.post_dim     #输入embedding的维度
